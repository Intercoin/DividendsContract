 ...
 */
contract TransferRules is Ownable, Whitelist, IHook {

	IERC777 public _erc777;
    ...
    modifier onlyHookCaller {
        require(msg.sender == address(hookCaller));
        _;
    }
    ...
    */
    function authorize(
        ...
        returns (bool) 
    {
        uint256 balanceOfFrom = IERC777(_erc777).balanceOf(from);
    ...
    */
    ...
    function doTransfer(
        ...
        onlyHookCaller()
    {
        uint256 balanceFromBefore = IERC777(_erc777).balanceOf(from);
        
        _applyRuleLockup(from, to, amount, balanceFromBefore);
        // store to daily amounts
        users[from].dailyAmounts[beginOfTheCurrentDay()] = users[from].dailyAmounts[beginOfTheCurrentDay()].add(amount);
        
        IHookCaller(hookCaller).executeTransfer(operator, from, to, amount, userData, operatorData);
    }
