...
var DividendsFactory = artifacts.require("DividendsFactory");
var DividendsGroupFactory = artifacts.require("DividendsGroupFactory");
...
contract('NFT', (accounts) => {
    ...
    before(async () => {
        ...
        //create instance to clone
        let DividendsContractInstance2Clone = await DividendsContract.new({ from: accountFive });
        let DividendsGroupContractInstance2Clone = await DividendsGroupContract.new({ from: accountFive });
        
        //create factories
        DividendsFactoryInstance = await DividendsFactory.new({ from: accountFive });
        DividendsGroupFactoryInstance = await DividendsGroupFactory.new({ from: accountFive });
    ...
    it('should produce instances by factories', async () => {
        ...
        tmpTr = await DividendsFactoryInstance.produceInstance(name, symbol, defaultOperators, interval, duration, multiplier, ERC20MintableInstanceToken.address, [ERC20MintableInstanceDividend.address]);
        assert.notEqual(
            (getArgs(tmpTr, "Produced").addr).toString(), 
            (zeroAddress).toString(), 
            'Empty address'
        );
        
        tmpTr = await DividendsGroupFactoryInstance.produceInstance(ERC20MintableInstanceToken.address, interval);
        assert.notEqual(
            (getArgs(tmpTr, "Produced").addr).toString(), 
            (zeroAddress).toString(), 
            'Empty address'
        );
        // var addr = getArgs(tmpTr, "Produced").addr; 
        // console.log(addr);
    });
    
    it('try to stake', async () => {
        tmpTr = await DividendsFactoryInstance.produceInstance(name, symbol, defaultOperators, interval, duration, multiplier, ERC777MintableInstanceToken.address, [ERC777MintableInstanceDividend.address]);
        let DividendsContractInstance = await DividendsContract.at(getArgs(tmpTr, "Produced").addr);
        
        let balanceBefore = await DividendsContractInstance.balanceOf(accountOne);
        
        // mint tokens and send to Dividends Contract
        await ERC777MintableInstanceToken.mint(accountOne, oneToken);
        await ERC777MintableInstanceToken.transfer(DividendsContractInstance.address, oneToken, {from: accountOne});
        
        let balanceAfter = await DividendsContractInstance.balanceOf(accountOne);

        assert.notEqual(
            balanceBefore.toString(),
            balanceAfter.toString(),
            'stake does not happens'
        );
        assert.equal(
    ...
    it('test group contract and disburse', async () => {
        tmpTr = await DividendsFactoryInstance.produceInstance(name, symbol, defaultOperators, interval, duration, multiplier, ERC777MintableInstanceToken.address, [ERC777MintableInstanceDividend.address]);
        let DividendsContractInstance = await DividendsContract.at(getArgs(tmpTr, "Produced").addr);
        
        
        tmpTr = await DividendsFactoryInstance.produceInstance(name, symbol, defaultOperators, interval*2, duration, multiplier, ERC777MintableInstanceToken.address, [ERC777MintableInstanceDividend.address]);
        let DividendsContractInstanceWithDoubleInterval = await DividendsContract.at(getArgs(tmpTr, "Produced").addr);
        
        tmpTr = await DividendsGroupFactoryInstance.produceInstance(ERC777MintableInstanceDividend.address, interval, {from: accountSix});
        let DividendsGroupInstance = await DividendsGroupContract.at(getArgs(tmpTr, "Produced").addr);
        
        
        await truffleAssert.reverts(
            DividendsGroupInstance.addDividendContracts([DividendsContractInstanceWithDoubleInterval.address], {from: accountSix}),
            "wrong interval in dividendsContract"
        );
        
        // check to illegal interval
        await truffleAssert.reverts(
            DividendsGroupInstance.addDividendContracts([DividendsContractInstance.address], {from: accountOne}),
            "Ownable: caller is not the owner"
        );
        
        // add dividendsContract to group
        await DividendsGroupInstance.addDividendContracts([DividendsContractInstance.address], {from: accountSix});
        
        // mint dividends token to group contract 
        await ERC777MintableInstanceDividend.mint(DividendsGroupInstance.address, twoToken);
        
        // make disburse from owner DividendsGroup when total shares in all dividends are zero
        await truffleAssert.reverts(
            DividendsGroupInstance.disburse(oneToken, {from: accountSix}),
            "total is equal zero"
        );
        ...
        // make disburse when totalShares != 0;
        await DividendsGroupInstance.disburse(oneToken, {from: accountSix});
    ...
    // it('stake, disburse, claim and redeem ', async () => {
    //     tmpTr = await DividendsFactoryInstance.produceInstance(name, symbol, defaultOperators, interval, duration, multiplier, ERC777MintableInstanceToken.address, [ERC777MintableInstanceDividend.address]);
    //     let DividendsContractInstance = await DividendsContract.at(getArgs(tmpTr, "Produced").addr);
